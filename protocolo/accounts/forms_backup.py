...
from stock.apps.common.fields import MyEmailInput
    ...
    def save(self, domain_override=None, email_template_name='registration/password_reset_email.html',
        ...
        for user in self.users_cache:
            ...
            c = {
                ...
                'domain': domain,
                'site_name': site_name,
                'uid': int_to_base36(int(str(user.pk), 16)),
                'user': user,
...
class UnidadeProfileForm(MongoForm):
    """
    Formulário de um Usuário de uma Unidade
    """

    def __init__(self, *args, **kwargs):
        super(UnidadeProfileForm, self).__init__(*args, **kwargs)

        self.fields['username'].widget.attrs.update({'minlength':'3'})
    ...
    def clean_username(self):
        ...
        if user and user != self.instance:
                raise forms.ValidationError(u"Este usuário já está cadastrado em nosso sistema")
    ...
    def clean_email(self):
        ...
        if user and user != self.instance:
            raise forms.ValidationError(u"Esse E-Mail já está cadastrado em nosso sistema")
    ...
    class Meta:
        ...
        exclude = ('password', 'is_superuser', 'last_login',
                   'date_joined', 'is_staff', 'user_permissions', )


class AddUnidadeProfileForm(UnidadeProfileForm):
    """
    Formulário para adicionar um Usuário de uma Unidade
    """
    password1 = forms.CharField(label="Senha", 
                                widget=forms.PasswordInput(attrs={'minlength':'4'}))
    password2 = forms.CharField(label="Senha Confirmação", 
                                widget=forms.PasswordInput(attrs={'minlength':'4',
                                                                  'equalTo':'#id_password1'}))
    ...
    def save(self, *args, **kwargs):
        user = super(AddUnidadeProfileForm, self).save(*args, **kwargs)
        user.set_password(self.cleaned_data["password1"])
