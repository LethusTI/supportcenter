	...
	$.fn.priceFormat = function(options) 
	{
		...
		return this.each(function() 
		{
			...
			function to_numbers (str) 
			{
				...
				{
					chr = str.charAt(i);
					if (formatted.length==0 && chr==0) chr = false;
					
					if (chr && chr.match(is_number))
					{
						if (limit) 
						{
							if (formatted.length < limit) formatted = formatted+chr;
						}
						else
						{
							formatted = formatted+chr;
						}
					}
				}
			...
			function price_format (str) 
			{
				...
				if (thousandsSeparator) 
				{
					...
					{
						chr = integerVal.substr(j-1,1);
						thousandsCount++;
						if (thousandsCount%3==0) chr = thousandsSeparator+chr;
						thousandsFormatted = chr+thousandsFormatted;
					}
